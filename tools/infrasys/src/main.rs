...
async fn check_infra_lock(toml_path: &PathBuf) -> Result<()> {
...
async fn create_infra(toml_path: &PathBuf, root_role_path: &PathBuf) -> Result<()> {
    ...
    for (repo_name, repo_config) in repos.iter_mut() {
        ...
        let prefix =
            s3::format_prefix(s3_info.s3_prefix.as_ref().context(error::MissingConfig {
                missing: format!("s3_prefix for '{}' s3 config", s3_stack_name),
            })?);
        ...
        // TODO: test adding bucket policy without the /*
...
mod tests {
    ...
    fn toml_yaml_conversion() -> Result<()> {
        ...
        let decoded_yaml = InfraConfig::from_path_yaml(&test_yaml_path).unwrap();
