...
impl InfraConfig {
    ...
    /// Deserializes an InfraConfig.lock to yaml format from a given path
    pub fn from_path_yaml<P>(path: P) -> Result<Self>
    where
        P: AsRef<Path>,
    {
        let path = path.as_ref();
        let infra_config_str = fs::read_to_string(path).context(error::File { path })?;
        serde_yaml::from_str(&infra_config_str).context(error::InvalidYaml { path })
    }
}
...
pub struct RepoConfig {
    ...
    pub root_key_threshold: Option<String>, 
    pub pub_key_threshold: Option<String>,
}
